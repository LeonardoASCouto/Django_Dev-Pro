...
@pytest.fixture
def video(db):
    v = Video(slug='motivacao', titulo='Vídeo Aperitivo: Motivação', vimeo_id='760589226')
    v.save()
    return v
...
def test_video_content(resp, video):
    assert_contains(resp,
                    f'<iframe src="https://player.vimeo.com/video/{video.vimeo_id}?h=ddccff4790&amp;badge=0&amp;autopause=0'
